...
import { X, Search, ChevronRight, Wallet, ArrowUpRight, ArrowDownLeft, Clock, User, ArrowLeft } from "lucide-react";
import { useState, useEffect } from "react";
import CustomerProfileModal from "@/components/customers/CustomerProfileModal";

export default function WalletsListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-800">Wallet Details</h2>
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div 
              ...
              className="bg-slate-50 hover:bg-slate-100/80 cursor-pointer transition-colors rounded-2xl p-6 mb-6 group border border-transparent hover:border-slate-200"
            >
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center">
                    <h3 className="text-lg font-bold text-slate-800 group-hover:text-teal-600 transition-colors">{item.customerName}</h3>
                    <User className="w-4 h-4 text-slate-400 ml-2 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                ...
                <div className="text-right">
                  ...
                  <p className="text-3xl font-black text-slate-800">₹{item.balance}</p>
                </div>
              </div>
            </div>

            <h4 className="text-sm font-bold text-slate-800 mb-4">Recent Transactions</h4>
            <div className="space-y-3">
              {[1, 2, 3].map((_, idx) => (
                <div key={idx} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-xl">
                  <div className="flex items-center space-x-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${idx === 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                      {idx === 0 ? <ArrowDownLeft className="w-5 h-5" /> : <ArrowUpRight className="w-5 h-5" />}
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-800">{idx === 0 ? 'Recharge' : 'Order Payment'}</p>
                  ...
                  <p className={`text-sm font-bold ${idx === 0 ? 'text-green-600' : 'text-slate-800'}`}>
                    {idx === 0 ? '+' : '-'}₹{idx === 0 ? '500' : '150'}
                  </p>
                </div>
              ))}
  ...
  const listItems = [].filter(item => {
    if (filterType === "active_wallets" && item.status !== "Active") return false;
    if (filterType === "recharges_today" && item.balance < 1000) return false; // Mock logic to differentiate
    if (filterType === "cashback_given" && item.balance >= 1000) return false; // Mock logic to differentiate
    
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      if (!item.customerName.toLowerCase().includes(q) && !item.phone.includes(q)) return false;
    }
    return true;
  });
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
          {listItems.length === 0 ? (
            ...
            listItems.map((item) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold border border-slate-200 group-hover:bg-teal-50 group-hover:text-teal-600 group-hover:border-teal-100 transition-colors">
                    {item.customerName.charAt(0)}
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-slate-800">{item.customerName}</h4>
                    <p className="text-[10px] font-medium text-slate-500 mt-0.5">{item.phone}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
