...
import { X, Search, ChevronRight, Tag, ArrowLeft, Settings2, Trash2 } from "lucide-react";
import { useState, useEffect } from "react";

export default function OffersListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-800">Coupon Details</h2>
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div className="bg-slate-50 rounded-2xl p-6 mb-6 flex justify-between items-center border border-slate-100">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Coupon Code</p>
                <div className="inline-flex items-center justify-center px-4 py-2 bg-white border border-slate-200 border-dashed rounded-xl">
                  <p className="text-2xl font-black text-teal-600 font-mono tracking-widest">{item.code}</p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Discount Details</p>
                  <p className="text-sm font-black text-slate-800">{item.discount}</p>
                  <p className="text-xs font-medium text-slate-500 mt-0.5">{item.type} Off</p>
                </div>
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Maximum Cap</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Usage Limit</p>
                  <p className="text-sm font-medium text-slate-700">
                    {item.usageLimit}
                  </p>
                </div>
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Total Redemptions</p>
                  <p className="text-sm font-black text-slate-800">
                    {item.redemptions} <span className="font-medium text-slate-500 ml-1 text-xs">times</span>
                  </p>
                </div>
              </div>
            </div>
  ...
  const listItems = [].filter(item => {
    if (filterType === "active_offers" && item.status !== "Active") return false;
            ...
            listItems.map((item) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-teal-600 font-mono">{item.code}</h4>
                    <p className="text-[10px] font-medium text-slate-500 mt-0.5">{item.discount} Off</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                    <span className="text-[10px] font-bold text-slate-500 block mb-1">Used: {item.redemptions}</span>
