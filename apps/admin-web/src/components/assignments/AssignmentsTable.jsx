...
export default function AssignmentsTable({ items, onRowClick }) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col mb-6">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="pb-3 pt-4 px-4 pl-6 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Assignment ID</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Order</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Rider</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Type</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Time</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="py-16 text-center text-slate-400 font-medium text-sm">No assignments found</td></tr>
            ) : filtered.map((item) => (
              <tr key={item.id} onClick={() => onRowClick && onRowClick(item)} className="cursor-pointer border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                ...
                <td className="py-4 px-4">
                  <span className="text-xs font-bold text-slate-500 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100">{item.type}</span>
                </td>
                <td className="py-4 px-4">
      ...
      <div className="xl:hidden p-4 space-y-3">
        {filtered.length === 0 ? (
          <div className="py-10 text-center text-slate-400 font-medium text-sm">No assignments found</div>
        ) : filtered.map((item) => (
