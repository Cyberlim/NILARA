...
import { X, Search, ChevronRight, ClipboardList, ArrowLeft, Clock, User, FileText } from "lucide-react";
import { useState, useEffect } from "react";

export default function AssignmentsListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-800">Assignment Details</h2>
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div className="bg-slate-50 rounded-2xl p-6 mb-6 flex justify-between items-start border border-slate-100">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Status</p>
                <div className="flex items-center">
                  <span className={`w-2 h-2 rounded-full bg-${item.statusColor}-500 mr-2`}></span>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Rider Assigned</p>
                ...
                <div className="p-4 border border-slate-100 rounded-xl">
                  ...
                  <p className="text-sm font-bold text-teal-600 flex items-center hover:underline cursor-pointer">
              ...
              <div className="p-4 border border-slate-100 rounded-xl">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Assignment Type</p>
                <span className="inline-flex px-3 py-1 bg-slate-100 rounded-lg text-xs font-bold text-slate-600">
  ...
  const listItems = [].filter(item => {
    if (filterType === "auto_assigned" && item.type !== "Auto") return false;
    if (filterType === "manual_assigned" && item.type !== "Manual") return false;
    if (filterType === "rejected" && item.status !== "Rejected") return false;
              ...
              >
                ...
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                    <span className="text-[10px] font-bold text-slate-400 block mb-1">{item.type}</span>
