...
export default function StaffTable({ items, onRowClick }) {
  ...
  const filtered = items.filter(item => {
    ...
    if (searchQuery) {
      ...
      if (!item.name.toLowerCase().includes(q) && 
          ...
          !item.role.toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col mb-6">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="pb-3 pt-4 px-4 pl-6 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Staff Member</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Role & Dept</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Last Active</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={5} className="py-16 text-center text-slate-400 font-medium text-sm">No staff members found</td></tr>
            ) : filtered.map((item) => (
              <tr key={item.id} onClick={() => onRowClick && onRowClick(item)} className="cursor-pointer border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                ...
                <td className="py-4 px-4">
                  <p className="text-sm font-medium text-slate-600">{item.lastActive}</p>
                </td>
                <td className="py-4 px-4">
      ...
      <div className="xl:hidden p-4 space-y-3">
        {filtered.length === 0 ? (
          <div className="py-10 text-center text-slate-400 font-medium text-sm">No staff members found</div>
        ) : filtered.map((item) => (
          <div key={item.id} onClick={() => onRowClick && onRowClick(item)} className="cursor-pointer bg-white rounded-2xl p-4 border border-slate-100 hover:shadow-md transition-shadow">
            ...
            <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-50">
              <div className="bg-slate-50 p-2 rounded-xl">
                <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">Department</p>
                <p className="text-xs font-medium text-slate-600">{item.department}</p>
              </div>
              <div className="bg-slate-50 p-2 rounded-xl text-right">
                <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">Last Active</p>
                <p className="text-xs font-medium text-slate-600">{item.lastActive}</p>
              </div>
            </div>
          </div>
        ))}
