...
export default function ReportsTable({ items, onRowClick }) {
    ...
    <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col mb-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between p-4 sm:p-6 border-b border-slate-50 gap-4">
        ...
        <button 
          ...
          className="flex items-center justify-center px-4 py-2 bg-white text-slate-600 border border-slate-200 rounded-xl text-sm font-bold hover:bg-slate-50 transition-colors w-full lg:w-auto"
        >
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="pb-3 pt-4 px-4 pl-6 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Report Name</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Category</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Generated On</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-right">Size</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="py-16 text-center text-slate-400 font-medium text-sm">No reports found</td></tr>
            ) : filtered.map((item) => (
              <tr key={item.id} onClick={() => onRowClick && onRowClick(item)} className="cursor-pointer border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                ...
                <td className="py-4 px-4">
                  <span className="text-xs font-medium text-slate-700 bg-slate-100 px-2.5 py-1 rounded-md">{item.category}</span>
                </td>
                <td className="py-4 px-4">
      ...
      <div className="xl:hidden p-4 space-y-3">
        {filtered.length === 0 ? (
          <div className="py-10 text-center text-slate-400 font-medium text-sm">No reports found</div>
        ) : filtered.map((item) => (
          <div key={item.id} onClick={() => onRowClick && onRowClick(item)} className="cursor-pointer bg-white rounded-2xl p-4 border border-slate-100 hover:shadow-md transition-shadow">
            ...
            <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-50">
              <div className="bg-slate-50 p-2 rounded-xl">
                <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">Generated</p>
                <p className="text-xs font-medium text-slate-600">{item.date}</p>
              </div>
              <div className="bg-slate-50 p-2 rounded-xl text-right flex flex-col justify-center items-end">
                {item.status === "Ready" ? (
                  <button className="flex items-center text-xs font-bold text-teal-600 bg-teal-50 px-3 py-1 rounded-lg">
