...
import { X, Search, ChevronRight, FileText, ArrowLeft, Download, Clock, Database, RefreshCw, Trash2 } from "lucide-react";
import { useState, useEffect } from "react";

export default function ReportsListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-800">Report Details</h2>
                <p className="text-sm font-medium text-slate-500 mt-1">{item.id}</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:text-teal-600 hover:bg-teal-50 shadow-sm transition-colors">
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div className="bg-slate-50 rounded-2xl p-6 mb-6 flex justify-between items-center border border-slate-100">
              <div className="flex items-center">
                <div className="w-12 h-12 rounded-xl bg-white border border-slate-200 flex items-center justify-center shadow-sm mr-4">
                  <FileText className="w-5 h-5 text-teal-600" />
                </div>
                <div>
                  <h3 className="text-lg font-black text-slate-800">{item.title}</h3>
                  <p className="text-sm font-medium text-slate-500 mt-1">{item.category} Report</p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border border-slate-100 rounded-xl bg-slate-50/50">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Generated On</p>
                ...
                <div className="p-4 border border-slate-100 rounded-xl bg-slate-50/50">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">File Size</p>
                  <p className="text-lg font-black text-slate-800 flex items-center">
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-between gap-3">
            ...
            <div className="flex gap-3">
              ...
              {item.status === 'Ready' && (
                <button className="px-6 py-2.5 bg-teal-600 text-white rounded-xl text-sm font-bold hover:bg-teal-700 transition-colors shadow-sm flex items-center">
  ...
  const listItems = [].filter(item => {
    // Basic filter map for KPIs, you can expand this based on precise logic
    if (filterType === "total_reports") return true; 
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-medium text-slate-500 mt-0.5">{item.category} • {item.date}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                    <span className="text-[10px] font-bold text-slate-500 block mb-1">{item.size}</span>
