...
export default function ExportLogModal({ isOpen, onClose }) {
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      ...
      alert(`Successfully generated ${format} export for ${status} reports (${dateRange}).\nDownloading now...`);
      onClose();
    }, 1500);
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-md bg-white/90 backdrop-blur-xl rounded-3xl border border-white/60 shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white/50">
          <div>
            <h2 className="text-lg font-black text-slate-800 tracking-tight">Export Report Logs</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">Generate export of your reports list</p>
          </div>
          <button 
            onClick={onClose}
