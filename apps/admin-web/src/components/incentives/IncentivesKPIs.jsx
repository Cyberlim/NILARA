"use client";

import { Zap, Wallet, Coins, Users } from "lucide-react";

export default function IncentivesKPIs({ kpis = {}, onCardClick }) {
  const cards = [
    {
      id: "active_campaigns",
      title: "Active Campaigns",
      value: kpis.activeCount ?? 0,
      trend: "Live for delivery partners",
      isPositive: true,
      icon: Zap,
      iconColor: "emerald",
      bgBadge: "bg-emerald-50",
      textBadge: "text-emerald-600",
      borderBadge: "border-emerald-100"
    },
    {
      id: "total_rewards",
      title: "Total Rewards Paid",
      value: `₹${(kpis.totalPaid || 0).toLocaleString()}`,
      trend: "Credited to rider wallets",
      isPositive: true,
      icon: Wallet,
      iconColor: "teal",
      bgBadge: "bg-teal-50",
      textBadge: "text-teal-600",
      borderBadge: "border-teal-100"
    },
    {
      id: "total_claims",
      title: "Completed Milestones",
      value: kpis.totalClaims ?? 0,
      trend: "Targets achieved",
      isPositive: true,
      icon: Coins,
      iconColor: "amber",
      bgBadge: "bg-amber-50",
      textBadge: "text-amber-600",
      borderBadge: "border-amber-100"
    },
    {
      id: "participating_partners",
      title: "Active Riders",
      value: kpis.participatingPartners ?? 0,
      trend: "Earning extra bonuses",
      isPositive: true,
      icon: Users,
      iconColor: "blue",
      bgBadge: "bg-blue-50",
      textBadge: "text-blue-600",
      borderBadge: "border-blue-100"
    }
  ];

  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      {cards.map((kpi) => {
        const Icon = kpi.icon;
        return (
          <div 
            key={kpi.id} 
            onClick={() => onCardClick && onCardClick(kpi.id)}
            className="cursor-pointer bg-white/80 backdrop-blur-xl rounded-3xl border border-white/60 shadow-[0_4px_20px_rgb(0,0,0,0.03)] p-5 flex flex-col justify-between transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1 hover:bg-white/95"
          >
            <div className="flex items-start space-x-3 mb-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${kpi.bgBadge} border ${kpi.borderBadge} shrink-0`}>
                <Icon className={`w-5 h-5 ${kpi.textBadge}`} />
              </div>
              <div className="flex-1 pt-0.5">
                <p className="text-[11px] sm:text-xs font-bold text-slate-500 mb-0.5 leading-tight break-words">{kpi.title}</p>
                <h3 className="text-xl font-black text-slate-800 tracking-tight">{kpi.value}</h3>
              </div>
            </div>
            <div className="flex items-center">
              <span className={`text-[11px] font-bold ${kpi.isPositive ? 'text-emerald-600' : 'text-red-600'}`}>
                {kpi.trend}
              </span>
            </div>
          </div>
        );
      })}
    </div>
  );
}
