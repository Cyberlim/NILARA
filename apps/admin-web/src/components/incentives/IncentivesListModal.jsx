...
import { X, Search, ChevronRight, Zap, ArrowLeft, Calendar, Target, Trophy } from "lucide-react";
import { useState, useEffect } from "react";

export default function IncentivesListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-800">Campaign Details</h2>
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div className="bg-slate-50 rounded-2xl p-6 mb-6 flex justify-between items-center border border-slate-100">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Reward</p>
                <div className="flex items-center">
                  <Trophy className="w-6 h-6 mr-2 text-amber-500" />
                  <p className="text-3xl font-black text-slate-800">{item.reward}</p>
                </div>
              </div>
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
            ...
            <button className="px-6 py-2.5 bg-slate-800 text-white rounded-xl text-sm font-bold hover:bg-slate-900 transition-colors shadow-sm">
              Edit Campaign
            </button>
          </div>
        </div>
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-medium text-slate-500 mt-0.5">{item.type}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                    <span className="text-sm font-black text-teal-600 block mb-1">{item.reward}</span>
