...
import { X, Search, ChevronRight, Banknote, ArrowLeft, Building2, User, FileText, Check, Clock } from "lucide-react";
import { useState, useEffect } from "react";
import CustomerProfileModal from "@/components/customers/CustomerProfileModal";

export default function WithdrawalsListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-800">Withdrawal Details</h2>
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div className="bg-slate-50 rounded-2xl p-6 mb-6 flex justify-between items-center border border-slate-100">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-2 flex items-center justify-end">
                  <Clock className="w-3 h-3 mr-1" /> {item.requestDate}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Account Ending In</p>
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
            ...
            {item.status === 'Pending' && (
              <>
                <button className="px-6 py-2.5 bg-red-50 text-red-600 border border-red-100 rounded-xl text-sm font-bold hover:bg-red-100 transition-colors shadow-sm">
                  Reject
                </button>
  ...
  const listItems = [].filter(item => {
    if (filterType === "pending_requests" && item.status !== "Pending") return false;
    if (filterType === "processed_today" && item.status !== "Approved") return false;
    if (filterType === "rejected_requests" && item.status !== "Rejected") return false;
              ...
              >
                ...
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                    <span className="text-sm font-black text-slate-800 block mb-1">₹{item.amount}</span>
