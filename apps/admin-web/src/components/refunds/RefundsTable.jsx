...
export default function RefundsTable({ items, setSelectedItem, setIsModalOpen, setModalMode }) {
  ...
  const filtered = items.filter(item => {
    ...
    if (searchQuery) {
      ...
      if (!item.id.toLowerCase().includes(q) && 
          ...
          !item.customer.toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col mb-6">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="pb-3 pt-4 px-4 pl-6 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Refund ID</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Order & Customer</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Reason</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="py-16 text-center text-slate-400 font-medium text-sm">No refunds found</td></tr>
            ) : filtered.map((item) => (
              <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                <td className="py-4 px-4 pl-6">
                  <span className="text-sm font-bold text-slate-800 font-mono tracking-widest bg-slate-100 px-2 py-1 rounded-md">{item.id}</span>
                  <p className="text-[10px] text-slate-400 mt-1.5">{item.requestDate}</p>
                </td>
                <td className="py-4 px-4">
                  <p className="text-sm font-bold text-slate-800">{item.customer}</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">{item.orderId}</p>
                </td>
                <td className="py-4 px-4">
      ...
      <div className="xl:hidden p-4 space-y-3">
        {filtered.length === 0 ? (
          <div className="py-10 text-center text-slate-400 font-medium text-sm">No refunds found</div>
        ) : filtered.map((item) => (
          <div key={item.id} className="bg-white rounded-2xl p-4 border border-slate-100 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-3">
              <div>
                <span className="text-xs font-bold text-slate-800 bg-slate-100 px-2 py-0.5 rounded-md font-mono">{item.id}</span>
                <p className="text-sm font-bold text-slate-800 mt-2">{item.customer}</p>
                <p className="text-[10px] text-slate-400 mt-0.5">{item.orderId}</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-50">
              <div className="bg-slate-50 p-2 rounded-xl">
                <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">{item.requestDate}</p>
                <p className="text-xs font-medium text-slate-600">{item.reason}</p>
              </div>
