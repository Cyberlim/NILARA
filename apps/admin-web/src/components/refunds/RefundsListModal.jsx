...
import { X, Search, ChevronRight, Banknote, ExternalLink, User, ArrowLeft } from "lucide-react";
import { useState, useEffect } from "react";
import CustomerProfileModal from "@/components/customers/CustomerProfileModal";

export default function RefundsListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-800">Refund Details</h2>
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div className="bg-slate-50 rounded-2xl p-6 mb-6 flex justify-between items-center">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Refund Amount</p>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 border border-slate-100 rounded-xl">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Reason for Refund</p>
                <p className="text-sm font-medium text-slate-700">{item.reason}</p>
              </div>
              
              <div className="p-4 border border-slate-100 rounded-xl">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Request Date</p>
                <p className="text-sm font-bold text-slate-800">{item.requestDate}</p>
              </div>
            </div>
          </div>
  ...
  const listItems = [].filter(item => {
    if (filterType === "processed_refunds" && item.status !== "Processed") return false;
