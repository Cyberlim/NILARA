...
export default function PartnersTable({ items, onRowClick }) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col mb-6">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="pb-3 pt-4 px-4 pl-6 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Partner Info</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Vehicle</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center">Rating</th>
              <th className="pb-3 pt-4 px-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center">Total Deliveries</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="py-16 text-center text-slate-400 font-medium text-sm">No delivery partners found</td></tr>
            ) : filtered.map((item) => (
              <tr key={item.id} onClick={() => onRowClick && onRowClick(item)} className="cursor-pointer border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                ...
                <td className="py-4 px-4">
                  <p className="text-sm font-bold text-slate-700">{item.vehicle}</p>
                </td>
                <td className="py-4 px-4 text-center">
                  <div className="flex items-center justify-center text-amber-500 font-bold text-sm bg-amber-50 w-max mx-auto px-2 py-0.5 rounded-lg">
                    <Star className="w-3.5 h-3.5 mr-1 fill-amber-500" />
                    {item.rating}
                  </div>
                </td>
                <td className="py-4 px-4 text-center">
                  <span className="text-sm font-black text-slate-800 bg-slate-100 px-3 py-1 rounded-full">{item.totalDeliveries}</span>
                </td>
                <td className="py-4 px-4">
      ...
      <div className="xl:hidden p-4 space-y-3">
        {filtered.length === 0 ? (
          <div className="py-10 text-center text-slate-400 font-medium text-sm">No delivery partners found</div>
        ) : filtered.map((item) => (
          <div key={item.id} onClick={() => onRowClick && onRowClick(item)} className="cursor-pointer bg-white rounded-2xl p-4 border border-slate-100 hover:shadow-md transition-shadow">
            ...
            <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-50">
              <div className="bg-slate-50 p-2 rounded-xl text-center">
                ...
                <div className="flex items-center justify-center text-amber-500 font-bold text-xs">
                  <Star className="w-3 h-3 mr-1 fill-amber-500" />
                  {item.rating}
                </div>
              </div>
              <div className="bg-slate-50 p-2 rounded-xl text-center">
                <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">Deliveries</p>
                <p className="text-xs font-black text-slate-800">{item.totalDeliveries}</p>
              </div>
            </div>
          </div>
        ))}
