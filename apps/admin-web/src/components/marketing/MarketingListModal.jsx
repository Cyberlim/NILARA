...
import { X, Search, ChevronRight, Megaphone, ArrowLeft, Target, Users, Settings2, Trash2 } from "lucide-react";
import { useState, useEffect } from "react";

export default function MarketingListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div className="bg-slate-50 rounded-2xl p-6 mb-6 flex justify-between items-center border border-slate-100">
              <div className="flex items-center">
                <div className="w-14 h-14 rounded-2xl bg-white border border-slate-200 flex items-center justify-center shadow-sm mr-4">
                  <Megaphone className="w-6 h-6 text-teal-600" />
                </div>
                <div>
                  <h3 className="text-xl font-black text-slate-800">{item.title}</h3>
                  <p className="text-sm font-medium text-slate-500 mt-1">{item.channel}</p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 border border-slate-100 rounded-xl bg-slate-50/50">
                  ...
                  <p className="text-2xl font-black text-teal-600">{item.openRate}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-medium text-slate-500 mt-0.5">{item.channel}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                    <span className="text-[10px] font-bold text-teal-600 block mb-1">Open: {item.openRate}</span>
