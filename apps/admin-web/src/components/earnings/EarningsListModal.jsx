...
import { X, Search, ChevronRight, Wallet, ArrowLeft, Download, User } from "lucide-react";
import { useState, useEffect } from "react";
import CustomerProfileModal from "@/components/customers/CustomerProfileModal";

export default function EarningsListModal({ isOpen, onClose, filterType, selectedItem, setSelectedItem, modalMode }) {
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-800">Earning Details</h2>
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar">
            <div className="bg-slate-50 rounded-2xl p-6 mb-6 flex justify-between items-center border border-slate-100">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-2">{item.date}</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div 
                onClick={() => {
                  setCustomerToView(item.riderName);
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Trips Completed</p>
                  <p className="text-xl font-black text-slate-800">{item.trips}</p>
                </div>
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Tips Collected</p>
                  <p className="text-xl font-black text-green-600">+₹{item.tips}</p>
                </div>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Distance Pay</p>
                  <p className="text-sm font-medium text-slate-800">₹{item.distancePay}</p>
                </div>
                <div className="p-4 border border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Trip Pay</p>
                  <p className="text-sm font-medium text-slate-800">₹{item.tripPay}</p>
                </div>
              </div>
            </div>
          </div>
  ...
  const listItems = [].filter(item => {
    if (filterType === "pending_clearance" && item.status !== "Pending") return false;
            ...
            listItems.map((item) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-800">{item.riderName}</h4>
                    <p className="text-[10px] font-medium text-slate-500 mt-0.5">{item.id}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                    <span className="text-sm font-black text-slate-800 block mb-1">₹{item.total}</span>
